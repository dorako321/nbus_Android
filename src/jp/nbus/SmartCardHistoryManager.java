...
public class SmartCardHistoryManager extends SQLiteOpenHelper {
	...
	public void putSmartCardHistory(SQLiteDatabase db,
			SmartCardHistory[] smartCardHistories, String idm) {

		// String idm = smartCardHistories[0].getIDm();

		Cursor c = db.rawQuery(
				"SELECT id,bytehistory,idm FROM bytehistories WHERE idm LIKE '"
						+ idm + "' ORDER BY id DESC LIMIT 1;", null);
		// 該当するIDmの一番最後に追加された履歴を見つける
		byte[] byteLastHistory = { (byte) 0x00, (byte) 0x00, (byte) 0x00,
		...
		if (i > 0) {// 追加すべき履歴配列上の要素が存在するときに以下の処理を実行
			c = db.rawQuery(
					"SELECT id FROM bytehistories ORDER BY id DESC LIMIT 1;",
					null);// idというカラムの最大値をとる(MAXを使わないのは、レコードがないときは結果を一件も返さないようにしたいため)
			int currentId;
			c.moveToFirst();
			if (c.getCount() > 0) {
			...
			c = db.rawQuery(
					"SELECT idm FROM idm WHERE idm LIKE '" + idm + "';", null);// IDmテーブルにIDmが存在するかどうか調べる
			...
			SQLiteStatement stmtByte = db
					.compileStatement("INSERT INTO bytehistories (id,bytehistory, idm) VALUES (?, ?, ?);");
			SQLiteStatement stmtCache = db
					.compileStatement("INSERT INTO historiescache (byteid, utiltype, date, time, stop, system, balance, fare, idm) VALUES (?,?,?,?,?,?,?,?,?)");
			SQLiteStatement stmtIDm = db
					.compileStatement("INSERT INTO idm (idm) VALUES (?);");
			try {
				for (int j = i - 1; j >= 0; j--) {// i:「履歴配列上の、DBに追加すべき履歴の“カウント”」
													// j:「履歴配列上でDBに追加すべき履歴の添え字」
					// 無いレコードの分だけ繰り返す
					...
					stmtCache.bindLong(6,
							smartCardHistories[j].getSystemOfPath());
	...
	public SmartCardHistory[] keepSmartCardHistory(
			SmartCardHistory[] smartCardHistories) {
		...
		byte[] emptyByte = { (byte) 0x00, (byte) 0x00, (byte) 0x00,
				(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00,
				(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00,
				(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00 };
		...
		SmartCardHistory[] cleanedSmartCardHistory = new SmartCardHistory[length
				- countEmpty];// 中身がある要素の数の分だけ配列を作成
		int i = 0;// iは新しい配列の添え字
		for (int l = 0; l < length; l++) {// lはもともとの配列の要素にアクセスするための添え字
			if (!Arrays.equals(smartCardHistories[l].getByteHistory(),
					emptyByte)) {// 空の要素ではないときに要素をコピー
				cleanedSmartCardHistory[i] = smartCardHistories[l];
				try {// 運賃計算
					if (l + 1 < length) {
						int fare = smartCardHistories[l + 1].getBalance()
								- smartCardHistories[l].getBalance();// 元々の配列で次の要素(一つ過去の履歴)の残高からいまの要素の残高を引く。運賃分は負、積み増し分は正になる。正負を逆転させるため*-1(変数がfareなので)
	...
	public String[][] idmList(SQLiteDatabase db) {
		Cursor c = db.rawQuery("SELECT id,idm,alias FROM idm ORDER BY id ASC;",
				null);
	...
	public ArrayList<TinyHistory> getHistory(SQLiteDatabase db, String idm) {
		Cursor c = db
				.rawQuery(
						"SELECT utiltype, date, time, balance, fare FROM historiescache WHERE idm LIKE '"
								+ idm + "' ORDER BY byteid DESC;", null);
		// String[] utiltypestring = new
		// String[]{Resources.getSystem().getString(R.string.card_util_undefined)};
