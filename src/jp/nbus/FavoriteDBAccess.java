...
public class FavoriteDBAccess {
	...
	public long addFavorite(int fm_id, String fm_name, String fm_ruby,
			int to_id, String to_name, String to_ruby, int co, String co_name) {
		// SQLiteHelperを作成
		FavoriteDBAccessHelper helper = new FavoriteDBAccessHelper(context,
	...
	public FavoriteRoutesAshDto[] readFavorites() {
		// SQLiteHelperを作成
		FavoriteDBAccessHelper helper = new FavoriteDBAccessHelper(context,
				DB_NAME, null, DB_VERSION);
		// SQLiteDatabaseを取得
		...
		String[] columnsName = { "id", "fm_id", "fm_name", "fm_ruby", "to_id",
				"to_name", "to_ruby", "co", "co_name" };
		// db.queryの返り値はCursor
		Cursor cursor = null;
		try {
			cursor = db.query("favorites", columnsName, null, null, null, null,
					null);
		...
		FavoriteRoutesAshDto[] favoriteRoutes = new FavoriteRoutesAshDto[count];
		for (int i = 0; i < count; i++) {
			FavoriteRoutesAshDto favoriteRoute;
			// fm(またはto)_idもしくはfm(またはto)_rubyもしくはcoもしくはco_nameのいずれかがnullのときはtrueになる変数(※DB上はINTEGERのカラムでもNULLが入る)
			boolean isSearchFavorite = cursor.isNull(1) || cursor.isNull(3)
					|| cursor.isNull(4) || cursor.isNull(6) || cursor.isNull(7)
					|| cursor.isNull(8);
			if (isSearchFavorite) {
				// 停留所名のみのコンストラクタ
				favoriteRoute = new FavoriteRoutesAshDto(cursor.getInt(0),
	...
	public void deleteFavorite(int rowId) {
		...
		try {
			...
			int rows = db.delete("favorites", "id=?",
	...
	public boolean IsDepulicate(int fmId, int toId, int co) {
		...
		Cursor cursor = null;
		try {
			cursor = db.query("favorites", columnsName,
					"fm_id=? and to_id=? and co=?",
					new String[] { String.valueOf(fmId), String.valueOf(toId),
							String.valueOf(co) }, null, null, null);
		} catch (NullPointerException e) {
			e.printStackTrace();
		}
		// cursorの先頭に移動
	...
	private class FavoriteDBAccessHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase dbarg) {
			...
			SharedPreferences sharedpreference = context.getSharedPreferences(
					"Nbus_Android", Context.MODE_PRIVATE);
